...
function DetailBar({ name, score, max, isWarning, rationale }) {
  ...
  return (
    <div className={`group relative flex items-center gap-2 py-1 px-2 rounded ${isWarning ? 'bg-amber-50' : ''}`}>
      <div className="w-16 text-xs text-gray-600 truncate cursor-help" title={rationale || name}>{name}</div>
...
export default function ScoreDetailPanel({ v22Details, dataGaps }) {
  ...
  return (
    <div className="p-6 border-b border-gray-200">
      ...
      <p className="text-xs text-gray-500 mb-3">💡 鼠标悬停在子项名称上可查看评分依据</p>
